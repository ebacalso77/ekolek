...
if (isset($_POST['register'])){
    ...
    if(mysqli_num_rows($select_email)>0){
        ...
        $insert=mysqli_query($con,"INSERT INTO `tbl_user` ( `username`, `password`, `email`, `fname`, `mname`, `lname`, `user_type`, `user_m_id`,brgy,phone,photo) VALUES ( '$username', '$encrypPass', '$email', '$fname', '$mname', '$lname', '2', '$municipality','$brgay','$phone','$uploadPath')");
        if ($insert){
            move_uploaded_file($fileTmp, $uploadPath);
            echo '<script>alert("Successfully created you user account.");window.open("../u/user.php","_self");</script>';
        }else{
            echo '<script>alert("Error encounter while creating your user account.")</script>';
        }
    }
}
?>
...
<head>
    ...
    <title>E-Kolek - User Register</title>

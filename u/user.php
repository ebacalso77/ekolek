...
if (isset($_POST['login'])) {// checking data input
    ...
    //checking data
    $user = mysqli_query($con, "SELECT * FROM tbl_user WHERE username='$username' and user_type=2 ");
    $countUser = mysqli_num_rows($user);
    $row=mysqli_fetch_assoc($user);
    if ($countUser == 0) {
        $output='<div class="alert alert-danger" role="alert">
  <strong>Error.</strong> User not register.
</div>';
    } else {
        session_start();
        if (sha1($password) === $row['password']) {
            ...
            echo '<script>window.open("user_main.php?id=' . $_SESSION['user_id'] . '","_self");</script>';
        }else{
            $output='<div class="alert alert-danger" role="alert">

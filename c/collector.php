...
if (isset($_POST['login'])) {// checking data input
    ...
    //checking data
    $user = mysqli_query($con, "SELECT * FROM tbl_user WHERE username='$username' and user_type=3 ");
    $countUser = mysqli_num_rows($user);
    $row=mysqli_fetch_assoc($user);
    if ($countUser == 0) {
        $output='<div class="alert alert-danger" role="alert">
  <strong>Error.</strong> Collector is not registered.
</div>';
    } else {
        session_start();
        if (sha1($password) === $row['password']) {
            ...
            echo '<script>window.open("collector_main.php?id=' . $_SESSION['user_id'] . '","_self");</script>';
        }else{
            $output='<div class="alert alert-danger" role="alert">
                        ...
                        <div class="col-lg-12" >
                            <div class="p-5">
                                <div class="text-center">
                                    <?=$output?>
                                    <h1 class="h4 text-gray-900 mb-4">Collector Login</h1>
                                </div>
                                ...
                                <br>
                                <div class="text-center">
                                    <a class="text-decoration-none text-dark " href="../index.php" title="Back to Main"><i class="fa fa-arrow-left fa-lg"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
